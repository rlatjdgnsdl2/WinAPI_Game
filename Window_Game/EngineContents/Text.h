...
public:
	...
	void SetString(std::string_view _StringValue, Color _Color = Color::White, float _Time = 0.0f, const FVector2D& _TextSize = FVector2D({ 14.0f,20.0f }));
	void SetString(const AText* _Text);
	void SetTime(float _Time) { Time = _Time; }

	void SetString(const std::vector<std::string>& _StringValues, const std::vector<Color>& _Colors, float _Time = 0.0f, const FVector2D& _TextSize = FVector2D({ 14.0f,20.0f }));
